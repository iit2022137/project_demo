...
import GreenCommerce from "./Components/GreenCommerce";
...
function App() {
  ...
  useEffect(() => {
    const apiCall = async () => {
      const response = await axios("https://raw.githubusercontent.com/RishitToteja/demo/main/products.json");
    ...
    <div>
      <Router>
        <Routes>
          ...
          />
          <Route
                path="/green-commerce" // add a new route for the green commerce page
                exact
                element={
                  <>
                    <NavBar />
